...
class WeatherChart extends StatelessWidget {
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              showForecast ? 'Forecast (48h)' : 'Precipitation History (5 days)',
  ...
  List<_ChartData> _prepareForecastData() {
    final now = DateTime.now();
    return weather.forecast.map((f) {
      final hoursDiff = f.date.difference(now).inHours;
      String label;
      if (hoursDiff < 24) {
        label = '${hoursDiff}h';
      } else {
        label = '${(hoursDiff / 24).floor()}d';
      }
      return _ChartData(
        label: label,
